<script lang="ts">
  ...
  let messages: { role: string; content: string; isGreeting?: boolean }[] =
    $state([]);
  let userInput = $state("");
  ...
  function loadMessages() {
    ...
    if (saved) {
      ...
      messages = [
        {
          role: "assistant",
          content: chatTrans.greeting || "Hello!",
          isGreeting: true,
        },
      ];
    }
  ...
  onMount(() => {
    ...
    if (typeof localStorage !== "undefined") {
      ...
      if (floating) {
        const hasAutoOpened = localStorage.getItem("chat_auto_opened");
        if (!hasAutoOpened) {
  ...
  let promptPills = $derived(
    Array.from(
      new Set(pills && pills.length ? pills : chatTrans?.prompt_pills || []),
    ) as string[],
  );
...
<div class="chat-wrapper" class:floating class:open={isOpen}>
  {#if floating}
    <button class="chat-trigger" onclick={toggleChat} aria-label="Toggle chat">
      <Icon
        icon={isOpen
          ? "material-symbols:close-rounded"
          : "streamline-sharp:chat-bubble-typing-oval"}
        width="34"
        height="34"
      />
    </button>
  ...
  <div class="chat-container-inner" class:full={!floating}>
      ...
      <div class="floating-header">
        ...
        <div class="header-info">
          <a
            href="/{lang}/chat"
            class="close-btn"
            title={chatTrans.full_chat || "Maximize"}
          >
            <Icon icon="material-symbols:chrome-maximize-outline" />
          </a>
          <button
            class="close-btn"
            title={chatTrans.close_chat || "Minimize"}
    ...
    <div
      ...
      class:startup={messages.length <= 1 && !floating}
    >
        ...
        <div class="prompt-carousel">
          <div class="prompt-track">
            {#each promptPills as pill}
              <button class="prompt-pill" onclick={() => usePrompt(pill)}>
                {pill}
              </button>
            {/each}
          </div>
    ...
    <div class="input-area">
      <div class="input-wrapper messageBox">
        ...
        <textarea
          id="messageInput"
          rows="1"
          title={chatTrans.placeholder ||
            place_holder ||
            "Type your message..."}
          placeholder={messages.length > 1
            ? chatTrans.placeholder2 || place_holder
            : chatTrans.placeholder || place_holder}
...
<style>
  ...
  .prompt-pill {
    ...
    cursor: pointer;
    transition: all 0.2s cubic-bezier(0.4, 1, 0.7, 1);
